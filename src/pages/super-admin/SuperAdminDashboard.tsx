...
import { Building2, Users, TrendingUp, DollarSign, Plus, Trash2, X, Bell, CreditCard, BarChart3, FileText, Settings as SettingsIcon } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-soft-sand animate-in fade-in duration-300">
      ...
      <div className="ml-64">
        ...
        {(activeTab === 'subscriptions' || activeTab === 'payments' || activeTab === 'systemLogs' || activeTab === 'settings') && (
          <div className="bg-white rounded-card shadow-sm border border-latte p-10 text-center">
            <div className="w-14 h-14 rounded-2xl bg-soft-sand flex items-center justify-center mx-auto mb-4">
              {activeTab === 'subscriptions' && <Bell className="w-6 h-6 text-espresso" />}
              {activeTab === 'payments' && <CreditCard className="w-6 h-6 text-espresso" />}
              {activeTab === 'systemLogs' && <FileText className="w-6 h-6 text-espresso" />}
              {activeTab === 'settings' && <SettingsIcon className="w-6 h-6 text-espresso" />}
            </div>
            <h3 className="text-lg font-bold text-espresso mb-2">{t(`navigation.${activeTab}`)}</h3>
            <p className="text-taupe text-sm max-w-md mx-auto">
              Bu bo'lim uchun backend'da hali tegishli API endpoint yo'q (Swagger hujjatida yo'q).
              Backend tomonda mos endpoint qo'shilgach, bu yerga real ma'lumot ulanadi.
            </p>
          </div>
        )}
        </div>
