...
export default function WaiterInterface() {
  ...
  // Savatdagi turli mahsulotlar soni (masalan: Osh + Salat = 2 ta mahsulot)
  const cartProductCount = cart.length;
  // Savatdagi umumiy dona/buyurtma soni (masalan: 3 ta Osh = 3 ta buyurtma)
  const cartQuantityTotal = cart.reduce((sum, item) => sum + item.quantity, 0);

  // Hali yakunlanmagan (kassaga o'tmagan) barcha buyurtmalar — pastki menyudagi
  // "Buyurtmalar" tugmasida son (badge) sifatida ko'rsatish uchun
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case "available":
        return "bg-gradient-to-br from-success to-sage text-white";
      case "occupied":
        return "bg-gradient-to-br from-terracotta to-danger text-white";
      case "cleaning":
        return "bg-gradient-to-br from-taupe to-gray-500 text-white";
      case "reserved":
        return "bg-gradient-to-br from-muted-gold to-latte text-white";
      default:
        return "bg-white border-2 border-soft-sand text-espresso";
    }
  };
            ...
            >
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-lg font-bold text-espresso">
                  Stol #{tables.find((t) => t.id === statusModalTable)?.number}{" "}
              ...
              {cart.length > 0 && (
                <div className="bg-white/10 backdrop-blur px-3 py-1 rounded-button">
                  <span className="font-bold">
                    {cartProductCount} ta mahsulot &bull; {cartQuantityTotal} ta
                    buyurtma
                  </span>
                </div>
              )}
  ...
  if (selectedTab === "orders" && !selectedTable) {
    ...
    // Yangi berilgan, hali oshxona qabul qilmagan/tayyorlamoqda bo'lgan buyurtmalar —
    // aynan shu qism yo'q edi, shuning uchun buyurtma berilgach hech qayerda ko'rinmasdi
    ...
    // Allaqachon stolga berib bo'lingan, lekin hali to'lanmagan buyurtmalar
                  ...
                  >
                    <div>
                      ...
                      <div className="text-sm text-taupe">
                        Buyurtma #{order.orderNumber} — {order.items.length} ta
                        mahsulot, {totalQty} ta buyurtma —{" "}
              ...
              {inProgressOrders.map((order) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className="px-3 py-1.5 rounded-button bg-soft-sand text-espresso font-bold text-xs flex-shrink-0">
            ...
            <div className="space-y-3 mb-4">
              {servedOrders.map((order) => {
                ...
                return (
                  <div
                    key={order.id}
                    className="bg-white rounded-card p-4 shadow-sm border-2 border-soft-sand flex items-center justify-between gap-4 opacity-80"
                  >
                    <div>
                      <div className="font-bold text-espresso">
                        Stol #{table?.number ?? "-"}
                    ...
                    <span className="px-3 py-1.5 rounded-button bg-latte text-espresso font-bold text-xs flex-shrink-0">
                      Berildi
                    </span>
                  </div>
                );
        ...
        {!hasAnyOrders && (
          <div className="flex-1 flex flex-col items-center justify-center p-8 text-center">
            ...
            <p className="text-lg text-taupe font-semibold mb-4">
              Hozircha faol buyurtmalar yo'q. Yangi buyurtma berish uchun stol
              tanlang
            </p>
            <button
...
function BottomNav({
  ...
  /** "Buyurtmalar" tugmasida ko'rsatiladigan faol buyurtmalar soni */
  ordersCount?: number;
}) {
  return (
      ...
      <div className="grid grid-cols-4 gap-1 p-2">
        {(
          [
            ...
            label: string;
            tab: "tables" | "orders" | "requests" | "more";
            icon: string;
          }[]
        ).map((item) => (
          <button
            ...
            className={`relative py-3 rounded-button transition-all ${
              selectedTab === item.tab
                ? "bg-gradient-to-r from-terracotta to-danger text-white font-bold shadow-lg"
                : "text-taupe hover:bg-soft-sand"
            }`}
          >
            {item.tab === "orders" && ordersCount > 0 && (
              <span className="absolute top-1 right-1/2 translate-x-4 min-w-[18px] h-[18px] px-1 rounded-full bg-danger text-white text-[10px] font-bold flex items-center justify-center shadow-md border border-white">
